...
export function BrandMark({ inverse = false }: { inverse?: boolean }) {
  return (
    <View className="flex-row items-center gap-3">
      ...
      <Text
        className={`font-display text-xl ${inverse ? "text-white" : "text-foreground dark:text-dark-foreground"}`}
      >
        GiveHub
      </Text>
    </View>
...
export function Screen({
  ...
  const content = (
    <View className={`flex-1 px-5 pb-8 pt-3 ${className}`}>{children}</View>
  );
  return (
    <SafeAreaView
      edges={["top"]}
...
export function Eyebrow({ children }: PropsWithChildren) {
  return (
    <Text className="mb-2 font-strong text-xs uppercase tracking-[1.7px] text-primary dark:text-dark-primary">
      {children}
    </Text>
  );
}
...
export function Display({
  ...
  return (
    <Text
      ...
      className={`font-display text-[34px] leading-[37px] tracking-[-1.2px] text-foreground dark:text-dark-foreground ${className}`}
    >
      {children}
    </Text>
  );
...
export function Heading({
  ...
  return (
    <Text
      accessibilityRole="header"
      className={`font-display text-2xl leading-8 tracking-[-0.7px] text-foreground dark:text-dark-foreground ${className}`}
    >
      {children}
    </Text>
  );
...
export function Body({
  ...
  return (
    <Text
      className={`font-sans text-sm leading-5 text-muted-foreground dark:text-dark-muted-foreground ${className}`}
    >
      {children}
    </Text>
  );
...
export function Card({
  ...
  return (
    <View
      className={`rounded-card border border-border bg-card p-5 dark:border-dark-border dark:bg-dark-card ${className}`}
    >
      {children}
    </View>
  );
...
export function Button({
  ...
  const colours =
    variant === "primary"
      ...
      : variant === "danger"
        ...
        : "border border-primary bg-transparent dark:border-dark-primary";
  const text =
    variant === "secondary"
  ...
  return (
    ...
    >
      {loading ? (
        <ActivityIndicator
          color={variant === "secondary" ? "#2A8D58" : "#F8FFF8"}
        />
      ) : (
        icon
      )}
  ...
  return (
    <View className="mb-4">
      ...
      <TextInput
        ...
        multiline={multiline}
        style={[
          multiline ? { paddingTop: 14, paddingBottom: 14 } : undefined,
          style,
        ]}
...
export function Chip({
  ...
  return (
    ...
    >
      <Text
        className={`font-medium text-sm ${selected ? "text-secondary-foreground dark:text-dark-secondary-foreground" : "text-foreground dark:text-dark-foreground"}`}
      >
        {label}
      </Text>
    </Pressable>
...
export function LoadingState({
  label = "Loading GiveHub…",
}: {
  label?: string;
}) {
  return (
    <View className="flex-1 items-center justify-center gap-3">
      <ActivityIndicator color="#2A8D58" />
      <Body>{label}</Body>
    </View>
  );
}

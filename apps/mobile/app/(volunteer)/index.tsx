...
import { Body, BrandMark, Card, EmptyState, ErrorState, LoadingState, Screen, useThemeColours } from "@/components/ui";
...
export default function VolunteerHome() {
  ...
  return (
    <Screen className="px-5 pb-6 pt-3" refreshing={query.isRefetching} onRefresh={() => query.refetch()}>
      <View className="flex-row items-center justify-between">
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Open preferences" onPress={() => router.push("/(volunteer)/settings")} className="h-10 w-10 items-center justify-center rounded-full border border-border bg-card dark:border-dark-border dark:bg-dark-card">
          <Ionicons name="options-outline" size={19} color={colours.primary} />
          {!profile?.search_location_label ? <View className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full border border-card bg-primary dark:border-dark-card dark:bg-dark-primary" /> : null}
        </Pressable>
      </View>
      ...
      <Pressable accessibilityRole="search" onPress={() => router.push("/(volunteer)/search")} className="mt-5 min-h-[52px] flex-row items-center rounded-card border border-border bg-card px-4 dark:border-dark-border dark:bg-dark-card">
        <Ionicons name="search" size={19} color={colours.primary} />
      ...
      <Card className="mt-6 rounded-[27px] border-primary/20 bg-secondary p-5 dark:border-dark-primary/20 dark:bg-dark-secondary">
        <View className="flex-row items-start gap-3">
          ...
          <View className="flex-1">
            ...
            <Body className="mt-2 text-foreground/70 dark:text-dark-foreground/75">
              {query.isError
                ? "We couldn’t reach GiveHub just now. Pull down to refresh."
                : `${query.data?.length ?? 0} activities match your causes, free time, and ${profile?.search_radius_km ?? 25} km travel radius.`}
            </Body>
            <Pressable onPress={() => router.push("/(volunteer)/discover")} className="mt-3 min-h-11 flex-row items-center">
              ...
              <Ionicons className="ml-2" name="arrow-forward" size={17} color={colours.primary} />
            </Pressable>
          </View>
        </View>
      </Card>
      ...
      {query.isError ? (
        ...
        <EventCard item={query.data[0]} />
      ) : (
        <EmptyState title="Nothing nearby yet" body="Try expanding your search radius in preferences." />
      )}
    </Screen>
  );
}

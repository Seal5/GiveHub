  ...
  return <FlatList className="bg-night" pagingEnabled snapToInterval={height} decelerationRate="fast" data={query.data} keyExtractor={(item) => item.id} renderItem={({ item }) => (
    <ImageBackground source={{ uri: item.image_url ?? undefined }} style={{ height }} resizeMode="cover">
      ...
      <View className="flex-1 justify-end px-5 pb-8">
        <View className="mb-4 flex-row gap-2"><View className="rounded-full bg-paper/90 px-3 py-1"><Text className="font-mono text-[10px] uppercase text-ink">{item.distance_km === null ? "Distance unavailable" : `${item.distance_km.toFixed(1)} km away`}</Text></View><View className="rounded-full bg-moss px-3 py-1"><Text className="font-mono text-[10px] uppercase text-white">{item.effort}</Text></View></View>
        <Text className="font-display text-[38px] leading-[42px] text-paper">{item.title}</Text><Text className="mt-3 font-sans text-base text-paper/75">{item.impact_statement}</Text><Text className="mt-4 font-medium text-paper">{formatEventDate(item.starts_at)} · {item.location_label}</Text>

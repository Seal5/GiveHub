...
export default function AuthScreen() {
  const { role: roleParam, mode } = useLocalSearchParams<{ role: string; mode?: string }>();
  ...
  return (
    <Screen className="px-6 pb-8 pt-4">
      ...
      {submitError ? <Text accessibilityRole="alert" className="mb-4 font-sans text-destructive dark:text-dark-destructive">{submitError}</Text> : null}
      {!signIn ? <View className="mb-5 rounded-card bg-secondary p-4 dark:bg-dark-secondary"><Text className="font-sans text-sm leading-5 text-foreground/75 dark:text-dark-foreground/75">{organiser ? "Organiser accounts are auto-approved during the beta; verification status is retained for launch." : "You control what hosts see in each application."}</Text></View> : null}
      <Button label={signIn ? `${organiser ? "Organiser" : "Volunteer"} sign in` : "Create account"} onPress={submit} loading={isSubmitting} />
      <Pressable onPress={() => setSignIn((value) => !value)} className="min-h-14 items-center justify-center"><Text className="font-strong text-sm text-primary dark:text-dark-primary">{signIn ? "Need an account? Create one" : "Already have an account? Sign in"}</Text></Pressable>
      {signIn ? <Pressable onPress={() => router.push("/auth/reset-password")} className="min-h-11 items-center"><Text className="font-sans text-sm text-muted-foreground dark:text-dark-muted-foreground">Forgot password?</Text></Pressable> : null}
    </Screen>
  );
}
